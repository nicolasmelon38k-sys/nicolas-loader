const fs = require('fs');
console.log("🚀 Iniciando Injeção Tycoon V6 (A Era dos 2nm)...");

// ==========================================
// 1. MOTOR BACKEND (Múltiplas Máquinas e Materiais)
// ==========================================
const backendV6 = `module.exports = function(app, checkAuth, db, configWeb) {
    const path = require('path');

    app.get('/tycoon', checkAuth, (req, res) => { res.sendFile(path.join(__dirname, '../views/tycoon.html')); });

    // 👨‍🔬 RH COM NOVOS ESPECIALISTAS
    const rhDB = {
    ...
    // 🏭 MÁQUINAS (UPGRADES)
    const upgradesDB = {
        "esteira": { nome: "Esteiras Magnéticas", desc: "Base para produção em massa", preco: 80000, icon: "fas fa-cogs", cor: "#ff9f0a" },
        "forno_dif": { nome: "Forno de Difusão", desc: "Dopagem térmica de silício", preco: 500000, icon: "fas fa-fire", cor: "#ff453a" },
        "maquina_cmp": { nome: "Máquina CMP", desc: "Polimento Químico-Mecânico", preco: 2500000, icon: "fas fa-compact-disc", cor: "#32d74b" },
    ...
    app.get('/api/empresa/info', checkAuth, (req, res) => {
        ...
        if (!emp.materiais) {
            emp.materiais = { silicio: emp.materia||0, fosforo: emp.fosforo||0, cobre: 0, ouro: 0, neodimio: 0 };
        }
        ...
        if (segPassados >= 1) {
            ...
            for(let i=0; i<limitador; i++) {
                ...
                // O Salário é cobrado MESMO se a fábrica estiver pausada! (Manutenção)
                if (emp.caixa >= custoSeg) {
                    emp.caixa -= custoSeg;
                    
                    if (emp.pausado) {
                        // Não faz nada, só pagou o salário
                    } else if(Object.keys(emp.equipe||{}).length === 0) {
                        addLog(emp, "Fábrica fantasma! Contrate alguém no RH.", "gold");
                    } else {
                        // Verifica Requisitos (Funcionários e Máquinas)
                        let temFuncs = chipProjeto.reqFunc.every(f => (emp.equipe[f] > 0));
                        let temUpgs = chipProjeto.reqUpg.every(u => emp.upgrades.includes(u));
                        
                        if (!temFuncs) {
                            addLog(emp, \`RH Insuficiente para o \${chipProjeto.nome}!\`, "red");
                        } else if (!temUpgs) {
                            addLog(emp, \`Faltam Máquinas para o \${chipProjeto.nome}!\`, "red");
                        } else {
                            // Verifica se tem todos os Materiais
                            let temMats = true;
                            for (let m in chipProjeto.mats) {
                                if ((emp.materiais[m]||0) < chipProjeto.mats[m]) temMats = false;
                            }
                            
                            if (temMats) {
                                // Consome materiais
                                for (let m in chipProjeto.mats) {
                                    emp.materiais[m] -= chipProjeto.mats[m];
                                }
                                emp.estoqueModelos[emp.chipAtual] = (emp.estoqueModelos[emp.chipAtual]||0) + 1;
                                addLog(emp, \`Fabricou 1x \${chipProjeto.nome} [\${chipProjeto.transistores}]\`, "green");
                            } else {
                                addLog(emp, "Faltam Materiais (Acesse a aba MATERIAIS)!", "gold");
                            }
                        }
                    }
                } else {
    ...
    app.post('/api/empresa/acao', checkAuth, (req, res) => {
        ...
        if (tipo === 'toggle_pause') {
            ...
            let info = matDB[valStr];
            let custoTotal = valor * info.preco;
            if (valor <= 0 || emp.caixa < custoTotal) return res.json({erro: \`Caixa insuficiente. Custa R$ \${custoTotal}\`});
            emp.caixa -= custoTotal; emp.materiais[valStr] = (emp.materiais[valStr]||0) + valor;
            addLog(emp, \`Estoque: +\${valor}g de \${info.nome}\`, 'white');
        }
        else if (tipo === 'mudar_chip') {
            emp.chipAtual = valStr;
            addLog(emp, \`Nova Linha: \${chipsDB[valStr].nome}\`, 'blue');
        }
        else if (tipo === 'vender') {
        ...
        db.salvar(idLimpo, user);
        res.json({ success: true, msg: "Ação executada." });
    });
};`;
fs.writeFileSync('./rotas_web/empresa.js', backendV6);

// ==========================================
// 2. TELA FRONTEND (Guias de Materiais e Máquinas)
// ==========================================
const frontendV6 = `<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
    <title>DaemonTech V6</title>
...
<body>
    ...
    <div class="content">
        <div class="section active" id="sec-painel">
            ...
            <div class="card" style="margin-top:10px; border-color:var(--blue); display:flex; justify-content:space-between; align-items:center;">
                <div>
                    <div class="c-title">STATUS DA FÁBRICA</div>
                    <div class="c-val" style="color:var(--blue); font-size:14px;" id="ui-status">RODANDO</div>
                </div>
            ...
            <div class="card" style="margin-top:10px;">
                <div class="c-title">LINHA DE MONTAGEM ATUAL</div>
                <div class="c-val" style="color:#fff; font-size:14px;" id="ui-projeto-atual">Carregando...</div>
            </div>
            ...
            <button class="btn btn-sacar" onclick="execAcao('vender')"><i class="fas fa-truck"></i> EXPORTAR E VENDER CHIPS</button>
        ...
        <div class="section" id="sec-projetos">
            <div style="font-size:11px; color:#888; text-align:center;">Modelos disponíveis para a linha de produção.</div>
        ...
        <div class="section" id="sec-rh">
            <div style="font-size:11px; color:#888; text-align:center;">Salários são cobrados a cada minuto de operação (ou pausa).</div>
            <div id="list-rh"></div>
        </div>
    ...
    <script>
        ...
        function render(d) {
            ...
            for(let key in db.materiais) {
                ...
                htmlMats += \`<div class="card" style="display:flex; flex-direction:column; align-items:center; text-align:center;">
                    <i class="\${m.icon}" style="font-size:24px; color:\${m.cor}; margin-bottom:5px;"></i>
                    <b style="font-size:12px;">\${m.nome}</b>
                    <div style="font-size:14px; font-weight:900; margin-bottom:10px;">\${fmt(qtd)}g</div>
                    <button class="btn" style="background:#222; color:#fff; width:100%; font-size:10px;" onclick="openMod('comprar_mat', '\${key}')">COMPRAR (R$\${m.preco}/g)</button>
                </div>\`;
            }
            ...
            for(let key in db.chips) {
                let c = db.chips[key]; let isAtivo = (e.chipAtual === key);
                let btn = isAtivo ? \`<button class="btn" style="background:var(--green); color:#000;" disabled>NA LINHA DE MONTAGEM</button>\` : \`<button class="btn" style="background:#fff; color:#000;" onclick="execAcao('mudar_chip', 0, '\${key}')">FABRICAR ESSE</button>\`;
                let reqF = c.reqFunc.map(f => db.rh[f].nome).join(', ');
                let reqM = c.reqUpg.map(u => db.upgrades[u].nome).join(' + ');
                ...
                htmlChips += \`<div class="list-item" style="\${isAtivo ? 'border-color:var(--green);' : ''}">
                    ...
                    <div class="li-info" style="font-size:11px;"><span><i class="fas fa-atom"></i> \${c.transistores}</span> <span style="color:var(--gold);"><i class="fas fa-tag"></i> Vende: R$ \${fmt(c.preco)}</span></div>
                    <div class="li-info"><b>Materiais por Und:</b> \${reqMatsList}</div>
                    <div class="li-info" style="color:#aa4444;">Requere: \${reqF} | Máquinas: \${reqM}</div>
                    \${btn}
                </div>\`;
            }
        ...
        function openMod(tipo, arg='') {
            ...
            if(tipo === 'comprar_mat') { document.getElementById('m-title').innerText = 'Comprar Material'; document.getElementById('m-desc').innerText = 'Digite a quantidade em gramas (Debita do Caixa)'; }
        }

        async function sendInputAcao() {
            let v = Number(document.getElementById('m-input').value)||0;
            document.getElementById('modal').style.display = 'none';
            execAcao(mType, v, mArg);
        }
...
fs.writeFileSync('./views/tycoon.html', frontendV6);
console.log('✅ FRONTEND V6 OK: Tabs completas, botão de Pausa, Abas de Materiais, e Chips de 1nm!');
